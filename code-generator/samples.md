...
{
    await rc.Authorize("username", "extension", "password");`);
    var result = await rc.Restapi(apiVersion).Account(accountId).A2pSms().Messages().Get(listA2PSMSParameters);
}
```
...
{
    await rc.Authorize("username", "extension", "password");`);
    var result = await rc.Restapi(apiVersion).Account(accountId).Extension(extensionId).MessageStore().Delete(deleteMessageByFilterParameters);
}
```
