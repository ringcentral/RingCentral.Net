    ...
    public class ConsumeTextTest
    {
        ...
        public async void UpdateGlipPostText()
        {
                   ...
                   ))
            {
                ...
                var foundIt = false;
                foreach (var group in (await rc.TeamMessaging().Chats().List()).records)
                {
                    foreach (var post in (await rc.TeamMessaging().Chats(group.id).Posts().List()).records)
                        if (post.creatorId == currentExtension.id.ToString() && post.text != null)
                        {
                            foundIt = true;

                            var newText = Guid.NewGuid().ToString();
                            var r = await rc.TeamMessaging().Chats(group.id).Posts(post.id)
